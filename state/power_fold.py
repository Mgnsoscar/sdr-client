...
# ── Derived-formula + bridge-param resolution over a flat {dest: value} state ────────────────
# These are pure (no Qt) so they live here beside the fold rather than in the ui.param_form
# widget module: they are the reusable core for resolving a signal's bridge-keyed --power params
# from a dict of values — used by the param form (over its widgets), the sequence step editor's
# clamp caption, and the sequence-level achievability pass (timeline_model). `ui.param_form`
# re-exports both names for its own callers.
